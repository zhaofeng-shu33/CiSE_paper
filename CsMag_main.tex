...
We consider a simple triangle counting algorithm and apply it to considerable large graph. The code is available at \url{https://github.com/zhaofeng-shu33/triangle_counting}.
...
\subsection{Using lab bare-metal server}
...
Cloud DevOps public agent provides useful agent for general purpose task and is not suitable to run long-time experiment. For this kind of experiment, self-hosted agent should be used. Self-hosted agent includes laptops, workstations, lab bare-metal server, paid cloud virtual machines etc.
For our triangle counting experiment, we use lab bare-metal server to run larger experiment. Since we use OpenMP to do some parallelism on algorithm level, the multi-core cpus help a lot in accelerating the experiment. Actually, our self-hosted agent is a head node in an HPC cluster and we use it to submit the job to computing node.
...
Our computing node has 32 CPUs, 128GB memory and we still need around 10 hours to run this experiment and the peak memory consumption is 18GB.

Our self-hosted experiment is triggered when we update the source code repository on GitHub. To achieve this, we connect the HPC head node (DevOps agent) with GitLab and make a repository mirror from GitHub. The log of each running for self-hosted agent is available publicly on GitLab.

\subsection{Using paid cloud infrastructure}
Some laboratories rent virtual machines from public cloud provider and do not deploy their bare-metal servers. In this case, Cloud DevOps can be incorporated directly into their existing workflow. 

The experiment we use is a sequence alignment algorithm applied to protein sequences. 
The code is available at \url{https://github.com/zhaofeng-shu33/ssw_experiment}. The experiment is actually run on a Serverless infrastructure, which supports high parallelism. Serverless infrastructure is charged by usage.
Since there are many concurrent experiments we need a client to coordinate them. The client program we used is run on public agent provided by Microsoft Azure. We first compile the source code and deploy it to AWS Lambda platform using Travis. Then we run the client program to invoke these functions.
The experiment finishes in 2 minutes. 

From this example we also notice that Cloud DevOps is not binded to a specific provider. We can use DevOps provided by Microsoft to trigger the experiments deployed on Amazon.

...
To illustrate this process, we make a simple experiment which uses OpenMP to parallel the program. The time complexity depends on the size of input data and the number of CPUs used. For medium-sized input data (about 0.5GB), we use a Travis public agent to run the experiment. The agent has 2 CPU cores and 7.5GB memory and our program consumes 3.1GB memory in peak and finishes in 4.3 minutes. Data is downloaded from a Git large file storage server and cached for later use. If we use 2.0GB dataset as input, 
experiment should be done in a self-hosted environment since the public virtual machine of DevOps is not equipped with enough resources. We use the HPC storage node to save the dataset and the  CPU-intensive job is submitted  to one computing node by self-hosted agent on the head node. With parallelism enabled on 32 CPUs, we still need around 10 hours to run this experiment and the peak memory consumption is 18GB.

For these two cases, the program is compiled from the same source code and the two experiments can be triggered simultaneously by updating source code to GitHub. To achive this, we connect the HPC head node (DevOps agent) with GitLab and make a repository mirror from GitHub. This combination increases the reproducibility of the project.

For the input data in self-hosted agent environment, there are some special problems practationer should pay attention to. Input data can be prepared beforehand and put outside of the code repository to save time and space. To In such case, loading the data using absolute path makes the experiment hard to reproduce by others. We suggest use symbol links or input argument to make some switches.

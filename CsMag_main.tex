...
If real-world dataset is needed instead of artificial dataset, external platform should be used for storage since users can not upload their data to DevOps public agent. Data is downloaded at the beginning of the experiment, instructed by the code. Similar to local running environment, dependency cache can be used to cache the dataset for later experiment running and no downloading is needed except for the first time. For deep learning domain, it is also necessary to save the model parameter, which is also quite large. 

...
\author{Xingzhi Niu}
\affil{University of Washington, Tacoma}
...
In the previous section, we briefly overview the common practice in DevOps and how it can be related with scientific experiment reproducibility. Different domains may still face different problems in practice. In this section, we use experiments from the domain of Machine Learning and High Performance Computing to show that how Cloud DevOps can be used to solve the reproducibility problems. We believe Cloud DevOps can be used in experiments of other domains as well. 

\subsection{Machine Learning}
Experiment Reproducibility is argued in ML community from different perspectives \cite{kegl2018ramp}. We notice that a platform called Sotabench, similar to DevOps service but dedicated to Deep Learning Reproducibility, has been online in recent month. Currently it only runs the prediction steps for limited public dataset. For more general domains of machine learning, it is preferable to use simpler and more flexible approach to resolve the reproducibility problem. In this subsection, we consider some aspects of reproducibility using an experiment in the domain of machine learning. 

If researchers develop a new algorithm for a specific domain, the workflow shown in Fig \ref{fig:cicdworkflow} can be further decomposed into two phases:
...
the reusable library can be deployed to public available package repository. In our experiment, we implement the algorithm in Python C extension. The platform of deployment is \url{pypi.org}. {\bf Table} \ref{tab:deploy} shows the deployment of this extension to pypi using DevOps. The deployment result shows that this library supports two versions of Python in a cross-platform way.
\begin{table}
\centering
...
For the running stage of our experiment, we just install the deployed package and run the actual experiment. The workflow specification for this stage  is similar with Listing \ref{lst:wd}. In \texttt{requirements.txt} file, we put the package name of our algorithm and other dependencies. We also use the \texttt{sacred} tool \cite{greff2017sacred} to manage the experiment logic and each running log can be checked out publicly on the DevOps provider we used (Travis).

If real-world dataset is needed instead of artificial dataset, external platform should be used for storage since users can not upload their data to DevOps public agent. Data is downloaded at the beginning of the experiment using the API of specific programming language. Similar to local running environment, dependency cache can be used to cache the dataset for later experiment running and no download is needed except for the first time. For deep learning domain, it is also necessary to save the model parameter somewhere on the public Internet, which is also quite large. 
...
To illustrate this process, we make a simple experiment which uses OpenMP to parallel the program. The time complexity depends on the size of input data and the number of CPUs used. For medium-sized input data (about 0.5GB), we use a Travis public agent to run the experiment. The agent has 2 CPU cores and 7.5GB memory and our program consumes 3.1GB memory in peak and finishes in 4.3 minutes. The input data is downloaded from GitHub large file storage server and cached for later use. If we use 2.0GB dataset as input, 
experiment should be done in a self-hosted environment since the public virtual machine of DevOps is not equipped with enough resources. We use the HPC storage node to save the dataset and the  CPU-intensive job is submitted  to a computing node. With parallelism enabled on 32 CPUs, we still need around 10 hours to run this experiment and the peak memory consumption is 18GB.

For these two cases, the program is compiled from the same source code and the two experiments can be triggered simultaneously by updating source code to GitHub. To achieve this, we connect the HPC head node (DevOps agent) with GitLab and make a repository mirror from GitHub. This combination increases the reproducibility of the project. The log of each running is available on GitLab and Travis respectively.

For the input data in self-hosted agent environment, there are some special problems practitioner should pay attention to. Input data can be prepared beforehand and put outside of the code repository to save time and space. To In such case, loading the data using absolute path makes the experiment hard to reproduce by others. We suggest use symbol links or input argument to make some switches.
Also, it is not transparent if loading data directly from self-hosted disk because others do not know whether it is original data or not. Checksum is a good way to guarantee this. We recommend researchers to verify the checksums of the data before preprocessing. By guaranteeing the data integrity can the experiment be more rigorous and convincing.

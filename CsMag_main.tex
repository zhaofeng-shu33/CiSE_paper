...
If real-world dataset is needed instead of artificial dataset, data needed to be downloaded at the beginning of the experiment on public agent. many public  and external platform should be used. 
Similar to local running environment, dependency cache can be used to cache the dataset for later experiment running and no downloading is needed except for the first time. For deep learning domain, it is also necessary to save the model parameter, which is also quite large. 

...
It should be noticed that not all DevOps infrastructure supports every combination of the above. For example, many platforms do not support connecting the local runner to the cloud or only allows limited. Below is the table to compare DevOps providers which researchers can use freely to a certain extent.
...
AppVeyor& Windows \& Linux support & unlimited public projects; 1 concurrent job with unlimitted time; 5 self-hosted jobs
 \\
 \hline
 Azure pipelines & cross-platform & 1 hosted job with 1800 minutes; 2GB build artifacts storage; 1 self-hosted CICD
 \\
 \hline
  Bitbucket& & 
 \\
